...
static int RegisterBuiltinFilterPolicies(ObjectLibrary& library,
  ...
  library.AddFactory<const FilterPolicy>(
      EvolveDummyFilterPolicy::kClassName(),
      [](const std::string& /*uri*/, std::unique_ptr<const FilterPolicy>* guard,
         std::string* /* errmsg */) {
        // Use null statistics here; caller can still construct directly with
        // NewEvolveDummyFilterPolicy(shared_stats) when needed.
        guard->reset(NewEvolveDummyFilterPolicy(/*statistics=*/nullptr));
        return guard->get();
      });
  library.AddFactory<const FilterPolicy>(
